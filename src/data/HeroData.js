export const heroMain = {
  reverse: true,
  inverse: false,
  topLine: {
    // text: 'Free',
    // location: 'Park City, Utah',
  },
  headline: "Find the best events",
  description: "",
  buttonLabel: "Study Guide",
  buttonLabel1: "Portfolio Matrix",
  imgStart: "start",
  img: "./assets/DISEC.jpg",
  start: "true",
};

const heroOne = {
  reverse: true,
  inverse: true,
  topLine: {
    text: "Free",
    location: "Park City, Utah",
  },
  headline: "UNGA-DISEC",
  description:
    " Deliberation upon the conflict in the Syrian region along with its impact on the socio-political conditions and western relations, with special emphasis on seeking ways to de-escalate the situation.",
  buttonLabel: "Study Guide",
  buttonLabel1: "Portfolio Matrix",
  href1: "./assets/UNGA-DISEC (1).pdf",
  download1: "UNGA-DISEC Study Guide.pdf",
  href: "./assets/UNGA-DISEC_PORTFOLIO (1).pdf",
  download: "UNGA-DISEC_PORTFOLIO.pdf",
  imgStart: "start",
  img: "./assets/DISEC.jpg",
  start: "true",
};

const heroTwo = {
  reverse: false,
  inverse: false,
  topLine: {
    text: "Paid",
    location: "Hard Rock Stadium, Miami, Florida",
  },
  headline: "UNGA-SPECPOL",
  description:
    " Deliberation on the recent violence in the Israeli-Palestine conflict with special emphasis on violation of ceasefire and the state of Jerusalem.",
  buttonLabel: "Study guide",
  buttonLabel1: "Portfolio Matrix",
  href1: "./assets/UNGA - SPECPOL Study Guide.pdf",
  download1: "UNGA_SPECPOL STUDY GUIDE",
  href: "./assets/UNGA - SPECPOL_Portfolio Matrix.pdf",
  download: "UNGA_SPECPOL PORTFOLIO MATRIX",
  imgStart: "start",
  img: "./assets/SPECPOL.jpg",
  start: "true",
};

const heroThree = {
  reverse: true,
  inverse: true,
  topLine: {
    location: "New Orleans, Louisiana",
    text: "Exclusive",
  },
  headline: "AIPPM",
  description:
    " Discussing the New Labour Laws 2022, its effect on industrial exploitation of employees, with emphasis on the exploitation of sex workers and their security.",
  buttonLabel: "Study Guide",
  buttonLabel1: "Portfolio Matrix",
  href1: "./assets/aippm study guide 1.pdf",
  download1: "AIPPM-STUDY GUIDE",
  href: "./assets/portfoliomatrix aippm.pdf",
  download: "AIPPM PORTFOLIO MATRIX",
  imgStart: "",
  img: "./assets/AIPPM.jpg",
  start: "true",
};

const heroFour = {
  reverse: false,
  inverse: false,
  topLine: {
    location: "Augusta National Gold Club, Georgia",
    text: "Paid",
  },
  headline: "International Press",
  description:
    "Presenting to you an independent body, comprising of fearless reporters who with their thorough research, well-placed questions and detailed reports, uncover the hidden truth.",
  buttonLabel: "Study Guide",
  buttonLabel1: "Portfolio Matrix",
  href1: "./assets/IP Study_Guide.pdf",
  download1: "IP-STUDY GUIDE",
  href: "./assets/IP_PORTFOLIO (1).pdf",
  download: "IP-PORTFOLIO",

  imgStart: "",
  img: "./assets/ip.jpg",
  start: "true",
};

export const heroData = [heroOne, heroTwo, heroThree, heroFour];
